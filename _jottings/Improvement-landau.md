...
\begin{align}
	\frac{{\rm d} H}{{\rm d} t}=\frac{{\rm d}\lambda}{{\rm d} t}\left(\frac{\partial H}{\partial\lambda}_{q,p},
\end{align}
对*周期运动*做平均得到
\begin{align}
	\overline{\frac{{\rm d} H}{{\rm d} t}}=\overline{\frac{{\rm d}\lambda}{{\rm d} t}}\,\overline{\left(\frac{\partial H}{\partial\lambda}\right)}_{q,p}.
\end{align}
注意这里对周期运动做平均, 因此认为 $\lambda$ 在周期内是近似不变的, 其相对变化率也极小. 但如果直接认为 ${\rm d}\lambda/{\rm d} t$ 为 0, 则这一近似过于粗糙, 退回到保守系统的情况, 因此仍将其作为一个常数小量保留. 作为一个常数小量, ${\rm d}\lambda/{\rm d} t$ 自然可以单独提出做平均.
...
\begin{align}
	...
	&+\left(\frac{\partial H}{\partial p}\right)_{q,\lambda}\[\left(\frac{\partial p}{\partial q}\right)_{\lambda,H}{\rm d} q+\left(\frac{\partial p}{\partial \lambda}\right)_{q,H}{\rm d} \lambda+\left(\frac{\partial p}{\partial H}\right)_{q,\lambda}{\rm d} H\].
\end{align}
...
将周期 $T$ 的表达式代入整理得到
\begin{align}
	\oint\[\left(\frac{\partial p}{\partial H}\right)_{q,\lambda}\overline{\frac{{\rm d} H}{{\rm d} t}}+\left(\frac{\partial p}{\partial\lambda}\right)_{q,H}\overline{\frac{{\rm d}\lambda}{{\rm d} t}}\]{\rm d} q=0,
\end{align}
因此得到系统存在一个近似不变量
\begin{align}
